...
import numpy as np
import glob
from transformer.Model import Embedding, AddPositionalEncoding, Stacked_Encoder, Stacked_Encoder_sc, Stacked_Decoder, Encoder, Decoder, MultiHead_Attn, FeedForward, Generator

##############################################################################################################
### Encoder_Decoder_sxsc_sc ######################################################################################
##############################################################################################################
class Encoder_Decoder_sxsc_sc(torch.nn.Module):
  ...
  def __init__(self, n_layers, ff_dim, n_heads, emb_dim, qk_dim, v_dim, dropout, share_embeddings, src_voc_size, tgt_voc_size, idx_pad):
    super(Encoder_Decoder_sxsc_sc, self).__init__()
    ...
    self.stacked_encoder_s = Stacked_Encoder(n_layers, ff_dim, n_heads, emb_dim, qk_dim, v_dim, dropout) ### encoder for src or srcXsim
  ...
  def forward(self, src, xtgt, tgt, msk_src, msk_xtgt, msk_tgt):
    ...
    z_xtgt = self.stacked_encoder_sc(z_src, xtgt, msk_src, msk_xtgt) #[bs,ls,ed]
  ...
  def encode(self, src, xtgt, msk_src, msk_xtgt):
    ...
    z_xtgt = self.stacked_encoder_sc(z_src, xtgt, msk_src, msk_xtgt) #[bs,lxt,ed]
    return z_src, z_xtgt
