  ...
  def translate_beam(self):
    ...
    while True:
      ...
      if lt == self.max_size - 1: #last extension (force <eos>)
        ...
        pad_eos = torch.logical_or(pref_idx==self.tgt_voc.idx_pad, pref_idx==self.tgt_voc.idx_eos).nonzero(as_tuple=False) #do not force words of hyps for which pref_idx[i] == idx_pad/idx_eos
